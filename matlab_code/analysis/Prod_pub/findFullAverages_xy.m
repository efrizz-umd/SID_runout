...
function [quantity_rowavg_full,quantity_colavg_full,quantity_rowstd_full,quantity_colstd_full] = ...
    findFullAverages_xy(ynumgrids,znumgrids,Ltp,storey,storex)
...
% % ----------- intput ----------- %
% - storex - a cell array of some values you want to average
% - storey - a cell array of some values you want to average
...
for j = 1:ynumgrids
    for k = 1:znumgrids
        tempc_quantity_full(k,:) = (storey{k,j}.^2+storex{k,j}.^2).^0.5;
    end
...
for k = 1:znumgrids
    for j = 1:ynumgrids
        tempr_quantity_full(j,:) = (storey{k,j}.^2+storex{k,j}.^2).^0.5;
    end
